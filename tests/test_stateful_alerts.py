...
# Configure webhook secret for tests
settings.alert_webhook_secret = "test-secret"
_AUTH_HEADER = {"X-Alert-Token": "test-secret"}
...
def test_stateful_alert_flow():
    ...
    payload_1 = {
        ...
        "alerts": [{
            ...
            "fingerprint": "fp-123",
            "startsAt": utcnow().isoformat() + "Z"
        }]
    }

    resp = client.post("/webhook/alert", headers=_AUTH_HEADER, json=payload_1)
    ...
    with Session(engine) as s:
        state = s.query(AlertStateRecord).filter_by(fingerprint="fp-123").first()
        state.last_investigated_at = utcnow() - timedelta(hours=25)
        s.commit()
    ...
    resp = client.post("/webhook/alert", headers=_AUTH_HEADER, json=payload_resolved)
    assert resp.status_code == 202
    data = resp.json()
